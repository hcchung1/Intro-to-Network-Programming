...
char player_choices[MAX_ROOMS][MAX_CLIENTS_PER_ROOM];
int reply[MAX_ROOMS][MAX_CLIENTS_PER_ROOM];
int enable = 0;
        ...
        if (index != -1) {
            ...
            if (room_host[room_number] == client_fd) {
                if (room_client_count[room_number] > 0) {
                    ...
                    snprintf(sendline, sizeof(sendline), "%s is now the new host of room %d.\n", room_client_names[room_number][0], room_number + 1);
                    broadcast_message(room_number, sendline, -1);
                } else {
                    // 房間無人，重置房間設置
                    ...
                    snprintf(sendline, sizeof(sendline), "Room %d has been cleared as all players have left.\n", room_number + 1);
                    broadcast_message(room_number, sendline, -1);
                }
            }
        }
...
void start_game(int room_number) {
    ...
    for (int round = 0; round < room_rounds[room_number]; round++) {
        ...
        while (remaining_players != 0) {
            ...
            for (int i = 0; i < room_max_players[room_number]; i++) {
                if (adv_status[i] == 1) {
                    if (player_choices[room_number][i] == 'N' || player_choices[room_number][i] == 'n') {
...
void handle_client_message(int client_fd, fd_set *all_fds, int *max_fd) {
    ...
    switch (stage[client_fd]) {
        case STAGE_INPUT_NAME:
            ...
            snprintf(sendline, sizeof(sendline), "Welcome, %s! Enter room number (1-%d):\n", client_names[client_fd], MAX_ROOMS);
            if (write(client_fd, sendline, strlen(sendline)) < 0) {
                perror("Write to client failed");
            }
        ...
        case STAGE_SELECT_ROOM: {
            ...
            if (chosen_room < 0 || chosen_room >= MAX_ROOMS) {
                snprintf(sendline, sizeof(sendline), "Invalid room number. Please enter a number between 1 and %d:\n", MAX_ROOMS);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
                break;
            ...
            if (room_ready[chosen_room] == 0 && room_client_count[chosen_room] > 0) {
                snprintf(sendline, sizeof(sendline), "Room %d is being set up. Try another room or wait.\n", chosen_room + 1);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
                break;
            ...
            if (room_client_count[chosen_room] >= MAX_CLIENTS_PER_ROOM) {
                snprintf(sendline, sizeof(sendline), "Room %d is full. Try another room.\n", chosen_room + 1);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
                break;
            ...
            printf("client_room:%d  chosenroom: %d  roomnumber: %d\n", client_rooms[client_fd], chosen_room, room_number);

            if (room_client_count[chosen_room] == 1) {
                // 第一位加入者成為房主
                room_host[chosen_room] = client_fd;
                snprintf(sendline, sizeof(sendline), "You are the host of room %d. Set the number of players (5-8):\n", chosen_room + 1);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
                ...
                snprintf(sendline, sizeof(sendline), "You are guest no.%d in room %d.\nWaiting for the host to set up the game.\n", guest_number, chosen_room + 1);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }

                // 廣播新玩家加入
                snprintf(sendline, sizeof(sendline), "%s has joined room %d as guest no.%d.\n", client_names[client_fd], chosen_room + 1, guest_number);
        ...
        case STAGE_SET_PLAYER_COUNT: {
            ...
            if (value >= 5 && value <= 8) {
                ...
                printf("value: %d  room_max_players[room_number]: %d", value, room_max_players[room_number]);
                snprintf(sendline, sizeof(sendline), "Player numbers set to %d. Please enter the round numbers (3-5):\n", room_max_players[room_number]);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
                stage[client_fd] = STAGE_SET_ROUNDS;
            } else {
                snprintf(sendline, sizeof(sendline), "Invalid input. Please enter a number between 5 and 8:\n");
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
            }
        ...
        case STAGE_SET_ROUNDS: {
            ...
            if (value >= 3 && value <= 5) {
                ...
                snprintf(sendline, sizeof(sendline), "Round numbers set to %ld. Room setup complete! Waiting for players.\n", value);
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
                ...
                if (room_client_count[room_number] == room_max_players[room_number]) {
                    snprintf(sendline, sizeof(sendline), "Room %d is now full! All players, please type 'ok' or 'ready' to prepare.\n", room_number + 1);
                    broadcast_message(room_number, sendline, -1);
                    for (int i = 0; i < room_client_count[room_number]; i++) {
                ...
                snprintf(sendline, sizeof(sendline), "Invalid input. Please enter a number between 3 and 5:\n");
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
            }
        ...
        case STAGE_ROOM_OPERATION:
            ...
            if (client_fd == room_host[room_number]) {
                ...
                broadcast_message(room_number, sendline, -1);
                for (int i = 0; i < room_client_count[room_number]; i++) {
                    stage[room_clients[room_number][i]] = STAGE_READY; // 切換到準備階段
                }
                room_ready[room_number] = 2; // 標記房間已準備
            } else {
                // 客人可以發送訊息或其他操作
                printf("room%d: %d/%d people now\n", room_number, room_client_count[room_number], room_max_players[room_number]);
        ...
        case STAGE_READY:
            ...
            if (strcasecmp(readline, "ok") == 0 || strcasecmp(readline, "ready") == 0) {
                ...
                if (index != -1) {
                    ...
                    if (all_ready) {
                        snprintf(sendline, sizeof(sendline), "All players are ready. Host, type 'gogo' to start the game.\n");
                        broadcast_message(room_number, sendline, -1);
                    }
                }
                ...
                snprintf(sendline, sizeof(sendline), "Invalid command. Type 'ok' or 'ready'.\n");
                if (write(client_fd, sendline, strlen(sendline)) < 0) {
                    perror("Write to client failed");
                }
            }
        ...
        case STAGE_RUNNING:
            readline[strcspn(readline, "\n")] = '\0';
            int index = -1;
            if ((strcasecmp(readline, "Y") == 0 || strcasecmp(readline, "y") == 0 || strcasecmp(readline, "N") == 0 ||strcasecmp(readline, "n") == 0) && enable == 1) {
                for (int i = 0; i < room_client_count[room_number]; i++) {
                    if (room_clients[room_number][i] == client_fd) {
                        index = i;
                        break;
                ...
                if (index != -1) {
                    player_choices[room_number][index] = readline[0];
                    reply[room_number][index] = 1;
                    snprintf(sendline, sizeof(sendline), "%s made the decision for this round.\n", client_names[client_fd]);
                    broadcast_message(room_number, sendline, client_fd);
                }
            } else if (enable == 0) {
                snprintf(sendline, sizeof(sendline), "Don't reply now.\n", client_names[client_fd]);
                write(sendline, sizeof(sendline), client_fd);
            }
